...
export const createComment =
  ...
  async (dispatch) => {
    ...
    try {
      const data = {
        ...newComment,
        postclassId: post._id,
        postclassUserId: post.user._id,
      };
      const res = await postDataAPI('classcomment', data, auth.token);
...
export const updateComment =
  ...
  async (dispatch) => {
    ...
    dispatch({ type: POST_TYPES.UPDATE_POST, payload: newPost });
    try {
      patchDataAPI(`classcomment/${comment._id}`, { content }, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const likeComment =
  ...
  async (dispatch) => {
    ...
    try {
      await patchDataAPI(`classcomment/${comment._id}/like`, null, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const unLikeComment =
  ...
  async (dispatch) => {
    ...
    try {
      await patchDataAPI(
        `classcomment/${comment._id}/unlike`,
        null,
        auth.token,
      );
    } catch (err) {
  ...
  async (dispatch) => {
    ...
    try {
      deleteArr.forEach((item) => {
        deleteDataAPI(`classcomment/${item._id}`, auth.token);
